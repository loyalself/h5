...
use app\billiards\model\GuessGamenumber;
use app\common\BaseController;
use app\billiards\model\GuessList;
use app\billiards\model\GuessWinner;
use app\billiards\model\GuessWinnerRangfen;
use app\billiards\model\GuessIsScore;
use app\billiards\model\GuessTotalScore;
use app\billiards\model\GameLive;
use app\billiards\model\GuessWinnerJujian;
use think\Db;
class Guess extends Common
{
     ...
     * 从这个页面传递进来的参数有game_id和game_number(大赛唯一编号),game_id对应game_live表里的主键id
    ...
    public function guess()
    {
        ...
        $game_id = input('param.game_id');              //获得比赛的主键id
        $game_number = input('param.game_number');      //获得场次编号
        //先将这场比赛的数据查询出来
        ...
        if(request()->isPost())
        {
            ...
            //$guess_winner['3_winner_odds'] = $winner_id_3;
            try{
            ...
            //$guess_winner_rangfen['3_rangfen_odds'] = $post['3_rangfen_odds']?:0;
    ...
    public function editGuess()
    {
        ...
        $guess_gamenumber_one = $this->db->table('guess_gamenumber')->where('game_id',$game_id)->find();
        ...
        $guess_winner_jujian_one = $this->db->table('guess_winner_jujian')->where('game_id',$game_id)->find();
        $guess_score_one = $this->db->table('guess_score')->where('game_id',$game_id)->find();
    ...
    public function guess_old()
    {
        ...
        if(request()->isPost())
        {
           ...
           try{
            ...
            if($res)
            {
                echo json_html($msg,1,$domain."/billiards/guess/guessList/game_id/".$game_id);
                exit;
            }
