...
class Sport extends BaseController
{
    ...
    {
        $list = Db::table('sport_name')->select();
    ...
    public function sport()
    {
        ...
        //若有修改
        $id = intval(input('id'));
        $one = Db::table('sport_name')->where('id',$id)->find();
        return view('',['one'=>$one]);
    }
    /**
     * 删除
    ...
    public function del()
    {
        if(request()->isPost())
        {
            $id = input('post.id');
            try{
                $res = Db::table('sport_name')->where('id',$id)->delete();
            }catch (\Exception $e){
                return json_show($e->getMessage(),0);
            }
            if($res)
    ...
    public function changeStatus()
    {
        if(request()->isPost())
        {
            $data = input('post.');
            try{
                $res = Db::table('sport_name')->where('id',$data['id'])->update(['is_show'=>$data['value']]);
            }catch (\Exception $e){
                return json_show($e->getMessage(),0);
            }
            if($res)
