...
use app\ymq\model\GuessGamenumber;
use app\common\BaseController;
use app\ymq\model\GuessList;
...
class Guess extends BaseController
{
    ...
    public function guess()
    {
        ...
        if(request()->isPost())
        {
            ...
            $realgame_data = Db::table('game_live')
                ->where('id',$game_id)
                ->field('game_number,play_a_name,play_b_name,start_time')
                ->find();
            $realgame_starttime = $realgame_data['start_time'];
            $game_number = $post['game_number'];
            $start_time = strtotime($post['start_time']);       //获取用户提交过来的竞猜开始时间
    ...
    public function editGuess()
    {
        ...
        $guessData = Db::table('guess_list')->where('id',$guess_id)->find();
        ...
        echo $play_a_id;
        echo $play_b_id;
        $play_a_name = Db::table('star_list')->where('id',$play_a_id)->value('c_name');
        $play_b_name = Db::table('star_list')->where('id',$play_b_id)->value('c_name');
        //查出后台人员添加过的比赛竞猜信息
    ...
    public function del()
    {
        if(request()->isPost())
        {
            ...
            halt($id);
            try{
                $res = Db::table('guess_list')->where('id',$id)->delete();
            }catch (\Exception $e){
