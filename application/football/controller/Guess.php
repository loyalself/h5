...
use app\football\model\GuessList;
use app\football\model\GuessWinner;
use app\football\model\GuessWinnerRangfen;
use app\football\model\GuessIsScore;
use app\football\model\GuessTotalScore;
use app\football\model\GameLive;
use think\Db;
class Guess extends Common
{
    ...
    public function guess()
    {
        ...
        if(request()->isPost())
        {
            ...
            $winner_id_3 = $post['3_winner_id'];
             ...
             }elseif (empty($winner_id_3))
             {
                 return $this->result('',0,'平局不能为空');
             }
            //整合guess_list表信息
            ...
            //都得分赔率表    guess_is_score
            $guess_is_score['game_id'] = $game_id;
            $guess_is_score['game_number'] = $game_number;
            $guess_is_score['1_score_odds'] = $post['1_score_odds'] ?: 0;   //1代表都进球
            $guess_is_score['2_score_odds'] = $post['2_score_odds'] ?: 0;   //2代表否
            try{
                $res = GuessIsScore::create($guess_is_score);
            }catch (\Exception $e){
                return $this->error($e->getMessage());
            }
    ...
    public function editGuess()
    {
        ...
        if(request()->isPost())
        {
            ...
            $guess_is_score['1_score_odds'] = $post['1_score_odds'] ?: 0;   //1代表都进球
            $guess_is_score['2_score_odds'] = $post['2_score_odds'] ?: 0;   //2代表否
            $guess_is_score['id'] = input('post.guess_is_score_id');
            try{
                $gsc = new GuessIsScore();
                $res = $gsc->allowField(true)->save($guess_is_score,$guess_is_score['id']);
            }catch (\Exception $e){
                return $this->error($e->getMessage());
            }
            //进球大小盘   guess_total_score
